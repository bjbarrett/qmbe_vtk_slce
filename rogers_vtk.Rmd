...
We wish to examine if and how social learning can evolve in temporally varying environments. 
...
With this value,, we can estimate the fitness of social leaners at the stationaty value of $\bar Q$. This is:
...
...algebra ensues where we multiply by polynomial denominator....
...
#phat <- (1-U*s*b/c)/(1-U) #equilibrium values for frequency of social learners
...
for (t in 1:timesteps){
  #wI[t] <- w0 + s*b - c #fitness individual learners
  ...
  #q[t+1] <- (1-u[t])*( (1-p[t])*s + p[t]*q[t]) + u[t]*0 #frequecncy of adaptive behavior
...
Next we can pot the evolutionary dynamics of our simulation:
